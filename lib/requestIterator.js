...
RequestIterator.prototype.rebuildRequest = function () {
  ...
  if (this.currentRequest) {
    ...
    let reqData = { ...this.currentRequest }
    let hyperidValue
    if (this.reqDefaults.idReplacement && typeof reqData.body === 'string') {
      hyperidValue = this.hyperid()
      reqData.body = reqData.body.replace(/\[<id>\]/g, hyperidValue)
      // sostituisci anche nel path se serve
      if (typeof reqData.path === 'string') {
        reqData.path = reqData.path.replace(/\[%3Cid%3E]/g, hyperidValue)
      }
    }
    data = this.requestBuilder(reqData, this.context)
    if (data) {
      this.currentRequest.requestBuffer = data
